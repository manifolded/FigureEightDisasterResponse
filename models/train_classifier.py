...
# genTokenTable()
# Takes the ml_model and the size of the nlp vocabulary, n_vocab, and passes
# every token in the vocabulary to predict().  The results are all concatenated
# together into what I call the 'token table'.  This takes a few minutes.
...
# genCanonTable()
# Takes the 'token table' generated by the function above and looks up the
# tokens in the vocabulary, 'vocab'.  Returns the compiled results, known as
# a 'canon table'.
def genCanonTable(tokenTable, vocab):
    result = []
    for i in range(36):
...
#        vocab = nlp_model['tfidfvectorizer'].vocabulary_
        ...
        n_voc = len(vocab)
        tokenTable = genTokenTable(ml_model, n_voc)
